  ...
  if (await soApprovalLink.isVisible()) {
      ...
      for (let i = 0; i < approverRows.length; i++) {
          ...
          if (cells.length > 5) {
              ...
              // Skip optional approvers as per requirement
              if (trimmedOptional === 'N') {
                  if (trimmedRole) approverRoles.push(trimmedRole);
                  isOptionals.push(trimmedOptional);
                  console.log(`Row ${i+1}: Role="${trimmedRole}", IsOptional="${trimmedOptional}" (Added)`);
              } else {
                  console.log(`Row ${i+1}: Role="${trimmedRole}", IsOptional="${trimmedOptional}" (Skipped as requested)`);
              }
          } else {
