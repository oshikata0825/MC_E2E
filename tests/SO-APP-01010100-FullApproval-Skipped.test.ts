...
/**
 * Sales Order Approval Chain Test (Skipped Optional)
 * This test handles the sequential approval of a Sales Order by multiple users.
 * It reads the Sales Order Number from approval_info.json created by SO-01010100-FullApproval-Skipped.test.ts.
 */

test('Response01010100-FullApproval-Skipped', async ({ page }) => {
  ...
  let targetSO = '';
  // Default sequence for Pattern B (Updated for Skipped case)
  let approverUsers = ['MCTest2']; 
  ...
  if (fs.existsSync(infoPath)) {
      try {
          ...
          if (info.approverRoles && info.approverRoles.length > 0) {
              ...
              // Updated: MCTest2 handles both roles when the optional one is skipped
              const roleToUserMap: Record<string, string> = {
                  '2-BU Team Leader': 'MCTest2',
                  '2-BU Team Leader/Department Manager': 'MCTest2',
